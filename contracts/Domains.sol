...
contract Domains is ERC721URIStorage {
  ...
  // Store values related to ech domain = dns records
...
constructor(string memory _tld) ERC721 ("Naija Name Service", "NNS") payable {
  ...
  console.log("%s name service deployed", _tld);
    ...
    function price(string calldata name) public pure returns(uint) {
        ...
        require(len > 0);
        if (len == 3) {
        return 0.0003 * 10**17; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic cause the faucets don't give a lot
        } else if (len == 4) {
        return 0.0002 * 10**17; // To charge smaller amounts, reduce the decimals. This is 0.3
        } else {
        return 0.0001 * 10**17;
        }
    }
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the Naija name service", "image": "data:image/svg+xml;base64,',
